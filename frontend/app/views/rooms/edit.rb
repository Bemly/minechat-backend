      ...
      div(class: "card") do
        form(action: "/rooms/#{@room["id"]}", method: "post") do
          ...
          div(class: "form-group") do
            label(for: "name") { "房间名称" }
            input(type: "text", id: "name", name: "room[name]", value: @room.dig("attributes", "name"))
          end
          div(class: "form-group") do
            label(for: "room_type") { "类型" }
            input(type: "text", id: "room_type", name: "room[room_type]", value: @room.dig("attributes", "room_type"))
          end
